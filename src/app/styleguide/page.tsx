...
// Фирменная палитра: синий (основной бренд) + жёлтый (только акцент) на мягко-белом фоне.
const brandSwatches = [
  { cls: "bg-primary", name: "primary", hex: "#173A8B" },
  { cls: "bg-primary-dark", name: "primary-dark", hex: "#0E2559" },
  { cls: "bg-primary-bright", name: "primary-bright", hex: "#2B5BD0" },
  { cls: "bg-accent", name: "accent", hex: "#F7C31C" },
  { cls: "bg-accent-dark", name: "accent-dark", hex: "#E0A400" },
];

const neutralSwatches = [
  { cls: "bg-background", name: "background", hex: "#F5F7FB" },
  { cls: "bg-white", name: "white", hex: "#FFFFFF" },
  { cls: "bg-border", name: "border", hex: "#E3E8F1" },
  { cls: "bg-muted", name: "muted", hex: "#5A6A85" },
  { cls: "bg-ink", name: "ink", hex: "#0F1B33" },
];

type Swatch = { cls: string; name: string; hex: string };

function Swatches({ scale }: { scale: Swatch[] }) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
      {scale.map((sw) => (
        <div key={sw.name} className="space-y-1">
          <div className={`h-16 rounded-lg border border-border ${sw.cls}`} />
          <p className="text-center text-xs font-medium text-ink">{sw.name}</p>
          <p className="text-center text-[11px] uppercase text-muted">{sw.hex}</p>
        </div>
      ))}
    </div>
  );
      ...
      <Section surface="dark">
        <Container>
          ...
          <p className="mt-4 max-w-2xl text-muted">
            Синий — основной бренд, жёлтый — только акцент, мягко-белый фон. Одни и те же компоненты
            аккуратно выглядят и на светлых, и на тёмных секциях благодаря семантическим токенам
            поверхностей.
          </p>
      ...
      <Section surface="light">
        <Container className="space-y-10">
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium">Бренд — синий + жёлтый акцент</p>
            <Swatches scale={brandSwatches} />
          </div>
          <div className="space-y-2">
            <p className="text-sm font-medium">Нейтральные (фон и текст)</p>
            <Swatches scale={neutralSwatches} />
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            >
              ...
              <p className="mt-1 text-sm text-muted">text-ink / text-muted / border-border</p>
            </div>
            <div data-surface="dark" className="rounded-2xl border border-border bg-background p-6">
              <p className="font-display font-semibold text-ink">Тёмная поверхность</p>
        ...
        <Container className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-muted">
              Приглушённый текст (muted) — подписи, пояснения, второстепенная информация.
            </p>
          </div>
        </Container>
      </Section>
        ...
        <Container className="space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Card className="p-6">
              ...
              <p className="mt-1 text-sm text-muted">bg-surface, border-border, тень card.</p>
            </Card>
            <Card interactive className="p-6">
              <p className="font-display font-semibold">Интерактивная</p>
              <p className="mt-1 text-sm text-muted">Поднимается при наведении.</p>
            </Card>
            <Card className="p-6">
              ...
              <p className="mt-1 text-sm text-muted">Пример карточки набора ТО.</p>
            </Card>
          </div>
        </Container>
      </Section>
        ...
        <Container className="space-y-10">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-4">
              <div className="h-20 w-20 rounded-lg border border-border bg-surface" />
              <div className="h-20 w-20 rounded-xl border border-border bg-surface" />
              <div className="h-20 w-20 rounded-2xl border border-border bg-surface" />
              <div className="h-20 w-20 rounded-3xl border border-border bg-surface" />
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            >
              <div className="h-20 w-28 rounded-xl bg-surface shadow-soft" />
              <div className="h-20 w-28 rounded-xl bg-surface shadow-card" />
              <div className="h-20 w-28 rounded-xl bg-surface shadow-elevated" />
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-4">
              <div className="h-16 w-16 rounded-xl bg-accent motion-safe:animate-fade-up" />
              <div className="h-16 w-16 rounded-xl bg-accent motion-safe:animate-fade-in" />
            </div>
          </div>
        </Container>
      </Section>
