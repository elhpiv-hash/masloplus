...
Форма на `/kontakty` отправляет заявку на серверный роут `POST /api/booking`:

- Валидация одной zod-схемой (`src/features/booking/schema.ts`) на клиенте **и** на сервере.
- Анти-спам: rate limiting эндпоинта, honeypot-поле, временная ловушка, опционально Яндекс SmartCaptcha.
- Доставка — в Telegram-бот владельца (`TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` в `.env`); email можно добавить позже (нужен SMTP-клиент). Если канал не настроен — заявка принимается, но не доставляется (в лог пишется предупреждение **без ПД**).
- **ПД не попадают в URL и в логи**; обязательный чекбокс согласия ведёт на `/policy`.

> **152-ФЗ:** для боевого запуска персональные данные клиентов должны храниться на хостинге в РФ. Если используете Vercel — приём и хранение заявок держите на российской стороне (свой бэкенд/бот на РФ-хостинге).

## Статус

Проект собирается по шагам (см. `PROMPTS.md` в корне рабочего стола). Готовы: каркас и тулинг, дизайн-система, шапка/подвал, главная, услуги с калькулятором, наборы ТО, галерея, контакты с формой записи, юр-страницы. Что нужно от заказчика для боевого запуска — в [`docs/owner-checklist.md`](docs/owner-checklist.md).
